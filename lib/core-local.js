  ...
  return new Promise((resolve, reject) => {
    ...
    if (cuf.isPathExist(option.localPath)) {
      if (!cuf.isDirectory(option.localPath)) {
        reject(cou.getError('Error: localPath is exists but is not a directory. [%s]', option.localPath))
      } else {
      ...
      if (option.autoCreateLocalPath) {
        ...
        reject(cou.getError('Error: localPath is not exists and autoCreateLocalPath is set to false. [%s]', option.localPath))
      }
    }
  })
}
