...
exports.askRemotePath = function (workingObject) {
  const question = [{
    name: 'remotePath',
    default: conf.DEFAULT_REMOTEPATH,
    type: 'string',
    description: 'Enter remote path'
  }]
  return new Promise((resolve, reject) => {
    if (workingObject.validatedOption.remotePath !== null) {
      resolve(workingObject)
    } else {
      return askQuestion(question)
        .then((result) => {
          workingObject.validatedOption.remotePath = result.remotePath
          resolve(workingObject)
        })
    }
  })
